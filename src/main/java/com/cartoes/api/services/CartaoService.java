...
public class CartaoService {
	...
	public Optional<Cartao> searchByNumber(String number) throws ConsistenciaException {
		log.info("Service: buscando o cartão de número: {}", number);
		Optional<Cartao> cartao = cartaoRepository.findByNumero(number);
		if (!cartao.isPresent()) {
			log.info("Service: Nenhuma cartão de número: {} foi encontrado", number);
			throw new ConsistenciaException("Nenhuma cartão com id: {} foi encontrado", number);
		}
		return cartao;
	}
}